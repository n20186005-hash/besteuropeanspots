...
checkList.forEach(kw => {
  // Convert everything to lowercase, remove spaces, dashes to find similarities
  const normalizedKw = kw.toLowerCase().replace(/[-\s]/g, '');
  const matches = attractions.filter(a => {
    const nName = a.name ? a.name.toLowerCase().replace(/[-\s]/g, '') : '';
    const nEng = a.englishName ? a.englishName.toLowerCase().replace(/[-\s]/g, '') : '';
    const nSlug = a.slug ? a.slug.toLowerCase().replace(/[-\s]/g, '') : '';
    return nName.includes(normalizedKw) || nEng.includes(normalizedKw) || nSlug.includes(normalizedKw);
  });
  
  if (matches.length > 1) {
    duplicates.push({ keyword: kw, matches: matches.map(m => ({ name: m.name, slug: m.slug, category: m.category })) });
  }
});
