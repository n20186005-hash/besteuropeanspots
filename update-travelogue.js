...
德国莱茵施泰因城堡・Rheinstein Castle・德国・莱茵兰
...
const enNames = travelogueList.split('\n')
  .filter(l => l.trim())
...
const zhNames = travelogueList.split('\n')
  .filter(l => l.trim())
...
for (const attr of attractions) {
  ...
  if (matchEn || matchZh) {
    if (!attr.category.includes('travelogue')) {
      attr.category.push('travelogue');
    }
    matchCount++;
  } else {
    // 移除不匹配的 travelogue 分类
    attr.category = attr.category.filter(c => c !== 'travelogue');
  }
}

fs.writeFileSync(jsonFile, JSON.stringify(attractions, null, 2), 'utf8');
console.log(`Matched and categorized ${matchCount} travelogue entries.`);